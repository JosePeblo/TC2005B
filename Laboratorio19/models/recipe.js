...
module.exports = class Recipe {
    ...
    static deleteOne(index) {
        return db.execute('DELETE FROM recipe WHERE id_recipe = ?', [index]);
    }

    static fetchUserRecipes(index, user) {
        return db.execute('SELECT * FROM recipe WHERE id_recipe = ? AND id_user = ?;', [index, user]);
    }
